...
class SDP_Views_Profile
{
    ...
    public static function get($request, $match)
    {
        ...
        if (array_key_exists('profileId', $match)) {
            return Pluf_Shortcuts_GetObjectOr404('SDP_Profile', $match['profileId']);
        }
        $profile = self::getProfileOfUser($user);
        if($profile === null){
            $profile = new SDP_Profile();
    ...
    public static function getProfileOfUser($user){
        $profiles = $user->get_sdp_profiles_list();
    ...
    public static function update($request, $match)
    {
        ...
        $profile = null;
        if (array_key_exists('profileId', $match)) {
            $profile = Pluf_Shortcuts_GetObjectOr404('SDP_Profile', $match['profileId']);
